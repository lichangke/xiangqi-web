...
## 14. 本轮已确认事项
1. `semanticTag` 与 `situationShift` 优先由决策 AI 顺带产出，演绎 AI 只消费，不再引入额外单独的标签 AI
2. 第一版不强依赖独立的 `boardStateSummary` 字段，而更强调 `situationShift` + `storyThreadSummary`
3. 特殊事件全部共用一套 schema
4. 特殊事件侧的 `eventSemanticTag` 需要收敛成固定枚举，并单独成文维护
5. 特殊事件 schema 中的 `extensions` 这一版先不限制成按事件类型的白名单字段
6. `tone` 与 `highlightLevel` 这一版先不再细化，先看效果
7. fallback 允许带少量固定世界观语气，但不做完整角色群像演绎

## 15. 当前待确认事项
1. `semanticTag` 的固定枚举第一版具体保留哪些
2. `storyThreadSummary` 的长度与保留轮数如何控制
3. fallback 允许保留哪些最小语气特征，既不出戏也不显得太平
